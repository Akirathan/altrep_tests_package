 ...
 * 
 *  Modified for purposes of integration into ALTREP testing framework.
 */
...
/**
 * Get class descriptor along with some instance data used for construction
 * of new altrep object (via R_new_altrep).
 */
static Rboolean file_exists(const char *fname)
{
    return open(fname, O_RDONLY) > 0;
}

void simplemmap_init_classes(DllInfo *dll)
{
    InitMmapIntegerClass(dll);
    InitMmapRealClass(dll);
}

SEXP simplemmap_new_instance()
{
    // First create instance via mmap_file and then get data from the instance.
    const char *fname = "altrep_tests_package/extdata/file.txt";
    if (!file_exists(fname)) {
	error("File should exist");
    }
    SEXP fname_sexp = ScalarString(mkChar(fname));
    Rboolean keep_pointer = TRUE;
		      ...
		      Rboolean serOK, Rboolean warn)
{
    ...
    void *p = mmap(NULL, sb.st_size, pflags, MAP_SHARED, fd, 0);
